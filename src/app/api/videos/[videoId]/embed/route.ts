...
export async function GET(_request: Request, context: RouteContext) {
  try {
    ...
    if (profileError) {
      return NextResponse.json({ error: "Unable to verify student access." }, { status: 500 });
    }
    ...
    if (enrollmentError) {
      return NextResponse.json({ error: "Unable to verify course access." }, { status: 500 });
    }
    ...
    return NextResponse.json({ error: "Unable to open video right now." }, { status: 500 });
  }
}
